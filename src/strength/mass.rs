...
use crate::{math::*, parameters::*, Error, ILoadMass, IModel, LoadMass, LoadingType};
...
#[derive(Clone)]
pub struct Mass {
    /// Постоянная масса корпуса и механизмов судна
    model: Rc<dyn IModel>,
...
impl Mass {
    ...
    /// * model - Постоянная масса корпуса и механизмов судна
    ...
    fn sum(&self) -> Result<f64, Error> {
        ...
        let lightship = self.model.mass(&Bound::Full, LoadingType::Hull)
            + self.model.mass(&Bound::Full, LoadingType::Equipment);
        let icing = self.icing_mass.mass(&Bound::Full)?;
        let wetting = self.wetting_mass.mass(&Bound::Full)?;
        let mass_sum = deadweight + lightship;
    ...
    fn values(&self) -> Result<Vec<f64>, Error> {
        ...
        for b in self.bounds.iter() {
            let hull = self.model.mass(&b, LoadingType::Hull);
            vec_hull.push(hull);
            let equipment = self.model.mass(&b, LoadingType::Equipment);
